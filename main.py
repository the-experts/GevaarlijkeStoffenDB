...
def embed_chunks_with_metadata(chunks_with_metadata):
    ...
    for chunk_data in chunks_with_metadata:
        ...
        if not chunk_data['content'] or not chunk_data['content'].strip():
            logger.warning(f"Skipping empty chunk {chunk_data['chunk_index']} from page {chunk_data['page_number']}")
            skipped_count += 1
            continue

        logger.info(f"Embedding chunk {chunk_data['chunk_index']} from page {chunk_data['page_number']}...")
        try:
            ...
            logger.error(f"Error embedding chunk {chunk_data['chunk_index']}: {e}")
            skipped_count += 1
            continue

    if skipped_count > 0:
        logger.warning(f"Skipped {skipped_count} empty or invalid chunks")
...
def process_and_store_pdf(pdf_path, db_connector, max_length=1000, overlap=100):
    ...
    if not pages:
        raise ValueError("PDF contains no extractable content (empty or unreadable)")

    logger.info(f"Extracted {len(pages)} pages")

    # Step 2: Create chunks with metadata
    logger.info("Step 2: Creating chunks with metadata...")
    ...
    logger.info(f"Stored {rows_affected} chunks in database")
